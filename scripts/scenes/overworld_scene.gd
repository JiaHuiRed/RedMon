...
# 260715 Red 按当前所在区域挑选战斗背景，对应 data/maps.json 的 bg 字段
const AREA_BG := {
	"青木村":  "res://assets/backgrounds/室外.png",
	"华灵草原": "res://assets/backgrounds/草原.png",
	"碧溪镇":  "res://assets/backgrounds/室外.png",
}
...
var _starter_pending:      String  = ""  # 260727 Red 待确认的御三家名字
const STARTER_TRIO := ["炎喵", "蓝蛇", "小竹熊"]
var _prof_event_shown: bool      = false  # 260726 Red 旧 starter 流已移除，保留兜底
var _granny_pos: Vector2       = Vector2.ZERO  # 260727 Red 阿婆位置，用于御三家事件前的强制拦截
var _granny_stopped: bool      = false  # 260727 Red 防止阿婆反复拦截
...
var _xiaoxia_spr: Sprite2D       = null   # 260715 Red 头目战怀旧NPC小霞（跑入动画用）
var _party_active: bool         = false  # 260709 Red 精灵管理界面打开时跳过输入

# 菜单
const MENU_W := 390;  const MENU_H := 510
const MENU_X := 1044; const MENU_Y := 30
...
func _register_scene_npc(spr: Node2D) -> void:
	...
	if str(spr.get_meta("npc_dialog", "")) == "junmei_boss_npc":
		...
		spr.set_meta("npc_tile", tile)
		_npc_nodes.append(spr)
		_add_collider(spr.global_position, Vector2(20, 20))
		return
	# npc / guard / sign
	spr.set_meta("npc_tile", tile)
	_npc_nodes.append(spr)
	_add_collider(spr.global_position, Vector2(24, 24))
	# 260727 Red 记录阿婆位置，用于御三家事件前的强制拦截
	if str(spr.get_meta("npc_dialog", "")) == "__granny_hint__":
		_granny_pos = spr.global_position
...
func _bg_for_area(area: String) -> String:
	return AREA_BG.get(area, "res://assets/backgrounds/草原.png")
...
func _physics_process(delta: float) -> void:
	...
	if moved:
		...
		_check_shenhe_town()
		_check_egg_hatch()
		# 260727 Red 阿婆强制拦截：拿了蓝秋秋但还没救教授时，走太远会被阿婆叫回来
		if not _granny_stopped and GameState.has_starter and not GameState.starter_trio_given and not _prof_event_shown and _granny_pos != Vector2.ZERO:
...
func _handle_lab_visit() -> void:
	...
	if GameState.items.get("精灵葫芦", 0) == 0:
		...
		lines.append("陈教授：%s，你回来了！这是三个精灵葫芦，出门探险必备，拿去用吧！" % GameState.player_name)
	if not GameState.has_running_shoes:
		GameState.has_running_shoes = true
		lines.append("林薇：%s，恭喜你拿到了第一只精灵！教授让我把这双跑步鞋给你——穿上会走得更快哦！\n\n获得了【跑步鞋】！" % GameState.player_name)
	else:
		var tier := GameState.caught_count / 10
		if tier > GameState.linwei_reward_tier:
			GameState.linwei_reward_tier = tier
			var reward := tier * 1000
			GameState.money += reward
			lines.append("林薇：哇，你已经捕捉了 %d 只精灵了！教授让我给你奖金 %dG 作为鼓励！\n\n获得了 %dG！" % [GameState.caught_count, reward, reward])
		else:
...
func _handle_junmei_boss() -> void:
	...
	_xiaoxia_spr.visible = true
	var marker = get_node_or_null("青木村/角色/小霞_停靠点")
	var target = marker.position if marker else Vector2(608, 264)
	var tw = create_tween()
...
func _confirm_starter_trio() -> void:
	...
	if chosen.is_empty():
		chosen = STARTER_TRIO[_starter_cursor]
	_starter_pending = ""
...
func _build_pcbox_detail() -> void:
	var pn = _pcbox.panel
	var CX = 345; var CY = 33; var CW = 900; var CH = 570
