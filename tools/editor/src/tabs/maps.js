...
export class MapsTab {
  ...
  rebuildFromSpecies(speciesData) {
    ...
    // Merge species encounters into existing data so manual weights are preserved
    const result = this.encountersData || { "_comment": "中央遇敌表——按 map_id 索引", "maps": {} };
    if (!result.maps) result.maps = {};
    ...
    for (const sp of speciesData) {
      ...
      for (const enc of sp.encounters) {
        ...
        activeCombos.add(`${mapId}:${sp.name}`);
        if (!result.maps[mapId]) {
          result.maps[mapId] = { encounter_rate: enc.rate || 0, mons: [] };
        } else if (enc.rate && !result.maps[mapId].encounter_rate) {
          result.maps[mapId].encounter_rate = enc.rate;
        }
        const existing = result.maps[mapId].mons.find(m => m.species === sp.name);
        if (existing) {
          if (enc.rate !== undefined) existing.weight = enc.rate;
        } else {
          result.maps[mapId].mons.push({ species: sp.name, weight: enc.rate || 0 });
        }
      }
    }
    // Remove species that no longer have encounters
    for (const mapId of Object.keys(result.maps)) {
      result.maps[mapId].mons = result.maps[mapId].mons.filter(m => activeCombos.has(`${mapId}:${m.species}`));
      if (result.maps[mapId].mons.length === 0) delete result.maps[mapId];
    }
  ...
  renderList(filter) {
    ...
    list.innerHTML = items.map(m =>
      `<div class="sidebar-item ${m.id === this.currentId ? 'active' : ''}" data-id="${m.id}">
         ...
         <span style="font-size:11px;color:var(--text-muted)">${escapeHtml(m.type||"")}</span>
      </div>`
    ).join("");
    ...
    this.container.innerHTML = `
      <div class="form-section">
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
             <label>名称</label><input type="text" id="mp-name" value="${escapeHtml(map.name)}" />
          </div>
          <div class="form-group">
            <label>类型</label>
          ...
          <div class="form-group">
             <label>场景文件</label><input type="text" id="mp-scene" value="${escapeHtml(map.scene_file||"")}" />
          </div>
          <div class="form-group full-width">
            <label>描述</label>
             <textarea id="mp-desc" rows="3">${escapeHtml(map.desc||"")}</textarea>
          </div>
          <div class="form-group">
            <label>道馆</label>
             <input type="text" id="mp-gym" value="${escapeHtml(map.gym||"")}" placeholder="如「翠竹道馆」" />
          </div>
          <div class="form-group full-width">
            <label>关联NPC（逗号分隔）</label>
             <textarea id="mp-npcs" rows="2">${escapeHtml((map.npcs||[]).join(", "))}</textarea>
          </div>
        </div>
      </div>
  ...
  async _renderEncSection(map) {
    ...
    body.innerHTML = `
       <datalist id="enc-species-list">${speciesNames.map(n => `<option value="${escapeHtml(n)}">`).join("")}</datalist>
      ...
      <table class="list-table" id="enc-mons-table">
        ...
        <tbody>
          ${entry.mons.map((m, i) => `
            <tr>
               <td><input type="text" list="enc-species-list" class="enc-mon-species" data-idx="${i}" value="${escapeHtml(m.species||"")}" /></td>
  ...
  onDelete() {
    ...
    if (!item || !confirm(`确认删除地图「${escapeHtml(item.name)}」？`)) return;
