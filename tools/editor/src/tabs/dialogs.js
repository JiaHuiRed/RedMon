...
export class DialogsTab {
  ...
  renderList(filter) {
    ...
    list.innerHTML = items.map(k =>
      `<div class="sidebar-item ${k === this.currentId ? 'active' : ''}" data-key="${k}">
        <span class="item-name">${SEC_LABELS[k] || k}</span>
  ...
  onDelete() {
    ...
    if (!confirm(`确认删除分组「${SEC_LABELS[this.currentId] || this.currentId}」？此操作不可撤销。`)) return;
    ...
    this.container.innerHTML = `
      <div class="form-section">
        <div class="section-header">
          <span class="form-section-title">编辑: ${secLabel}（${key}）</span>
    ...
    if (Array.isArray(val)) {
        ...
        <div class="dl-list" data-dl-list="${k}">
          ${val.map((item, i) => `
            <div class="dl-list-row" style="display:flex;gap:6px;align-items:flex-start;margin-bottom:4px">
              ...
              <textarea class="dl-list-item" data-key="${k}" data-idx="${i}" rows="2" style="flex:1">${item ?? ""}</textarea>
      ...
      bodyHtml = Object.entries(val).map(([subK, subV]) => `
        <div style="margin:6px 0 6px 12px">
          <label style="font-size:11px;color:var(--text-muted)">${subK}</label>
          <textarea class="dl-nested" data-key="${k}" data-subkey="${subK}" rows="2" style="width:100%">${
            (subV && typeof subV === "object") ? JSON.stringify(subV, null, 2) : (subV ?? "")
          }</textarea>
        </div>
      `).join("");
    } else {
      bodyHtml = `<textarea class="dl-string" data-key="${k}" rows="3" style="width:100%">${val ?? ""}</textarea>`;
    }

    return `
      <div class="form-section dl-card" data-card-key="${k}">
        <div class="section-header">
          <span class="form-section-title" style="color:var(--accent, inherit)">${label}（${k}）</span>
