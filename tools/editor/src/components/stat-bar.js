...
// 与 mon_editor.py 的 _refresh_stat_bars 保持一致：Lv50/Lv120 的真实数值（种族值×努力值×个体值均取满时的近似展示）
...
export function renderStatBar(label, value, max = MAX_STAT) {
  ...
  const { lv50, lv120 } = calcLvStat(value, label === "hp");
  return `
    <div class="stat-row">
      ...
      <span class="stat-lv" title="Lv50 / Lv120 实际数值">${lv50} / ${lv120}</span>
    </div>
  `;
}
