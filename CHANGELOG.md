...
#### Changed
- **编辑器属性克制面板重构**：属性克制从右侧面板技能池下方移至底部，与进化链对比栏并排显示；新增攻击克制显示（克制/抵抗），展示当前精灵属性对哪些防御类型有效/无效
- **编辑器进化链对比重排**：分支进化改为同排横列，每个分支独立显示完整起始路径（`[当前] [当前]→Lv? [分支]`），填满横向布局空间
- **编辑器右侧面板可滚动**：右侧面板添加垂直滚动条，确保精灵预览/技能池/属性克制全部可见
